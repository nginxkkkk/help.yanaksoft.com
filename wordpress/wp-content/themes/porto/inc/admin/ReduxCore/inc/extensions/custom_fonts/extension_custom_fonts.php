...
if ( ! class_exists( 'ReduxFramework_extension_custom_fonts' ) ) {
	...
	class ReduxFramework_extension_custom_fonts {
		...
		public function __construct( $parent ) {
			...
			add_filter(
				"redux/{$this->parent->args['opt_name']}/field/typography/custom_fonts1",
				array(
					$this,
					'get_field_custom_fonts',
				)
			);			
					...
					if ( $section['type'] == 'd' && ! empty( $section['name'] ) ) {
						...
						foreach ( $section['files'] as $font ) {
							...
							if ( ! empty( $font['name'] ) ) {
								...
								$cf_label = 'Regular';
								$cf_name   = $font['name'];
								if ( false !== strpos( $font['name'], '-700' ) || false !== strpos( $font['name'], '-bold' ) ) {
									$cf_label = 'Bold';
									$cf_weight = '700';
									$cf_name = substr( $font['name'], 0, strrpos( $font['name'], '-' ) );
								} elseif ( false !== strpos( $font['name'], '-800' ) || false !== strpos( $font['name'], '-extrabold' ) || false !== strpos( $font['name'], '-ultrabold' ) ) {
									$cf_weight = '800';
									$cf_label = 'Extra Bold';
									$cf_name = substr( $font['name'], 0, strrpos( $font['name'], '-' ) );
								} elseif ( false !== strpos( $font['name'], '-900' ) || false !== strpos( $font['name'], '-black' ) ) {
									$cf_weight = '900';
									$cf_label = 'Black';
									$cf_name = substr( $font['name'], 0, strrpos( $font['name'], '-' ) );
								} elseif ( false !== strpos( $font['name'], '-600' ) || false !== strpos( $font['name'], '-semibold' ) ) {
									$cf_weight = '600';
									$cf_label = 'Semi Bold';
									$cf_name = substr( $font['name'], 0, strrpos( $font['name'], '-' ) );
								} elseif ( false !== strpos( $font['name'], '-500' ) || false !== strpos( $font['name'], '-medium' ) ) {
									$cf_weight = '500';
									$cf_label = 'Medium';
									$cf_name = substr( $font['name'], 0, strrpos( $font['name'], '-' ) );
								} elseif ( false !== strpos( $font['name'], '-400' ) || false !== strpos( $font['name'], '-regular' ) ) {
									$cf_weight = '400';
									$cf_name = substr( $font['name'], 0, strrpos( $font['name'], '-' ) );
								} elseif ( false !== strpos( $font['name'], '-300' ) || false !== strpos( $font['name'], '-light' ) ) {
									$cf_weight = '300';
									$cf_label = 'Light';
									$cf_name = substr( $font['name'], 0, strrpos( $font['name'], '-' ) );
								} elseif ( false !== strpos( $font['name'], '-200' ) || false !== strpos( $font['name'], '-extralight' )|| false !== strpos( $font['name'], '-ultralight' )  ) {
									$cf_weight = '200';
									$cf_label = 'Extra Light';
									$cf_name = substr( $font['name'], 0, strrpos( $font['name'], '-' ) );
								} elseif ( false !== strpos( $font['name'], '-100' ) || false !== strpos( $font['name'], '-thin' ) ) {
									$cf_weight = '100';
									$cf_label = 'Thin';
									$cf_name = substr( $font['name'], 0, strrpos( $font['name'], '-' ) );
								}
								if ( false !== strpos( $font['name'], 'italic' ) ) {
									$cf_style = 'italic';
									$cf_label .= ' Italic';
									if ( false !== strpos( $cf_name, '-italic' ) ) {
										$cf_name = substr( $cf_name, 0, strrpos( $cf_name, '-' ) );
									}
								}
								if ( empty( $this->custom_fonts[ $section['name'] ][ $cf_name ] ) ) {
									$this->custom_fonts[ $section['name'] ][ $cf_name ] = array();
								}

								$this->custom_fonts[ $section['name'] ][ $cf_name ][ $cf_weight . $cf_style ] = $cf_label;
		 ...
		 * @since 7.1.0
		 */
		public function get_field_custom_fonts() {

			$field_custom_fonts = array();
			...
			if ( ! empty( $fonts ) ) {
				foreach ( $fonts as $section ) {
					if ( $section['type'] == 'd' && ! empty( $section['name'] ) ) {
						...
						$field_custom_fonts[ $section['name'] ] = isset( $field_custom_fonts[ $section['name'] ] ) ? $field_custom_fonts[ $section['name'] ] : array();
						...
						foreach ( $section['files'] as $font ) {
							...
							if ( ! empty( $font['name'] ) ) {
								...
								$field_custom_fonts[ $section['name'] ][ $font['name'] ] = $kinds;
		...
		private function generateFontCSS( $name, $dir ) {
			...
			$cf_style  = 'normal';
			$cf_name   = $name;
			if ( false !== strpos( $name, '-700' ) || false !== strpos( $name, '-bold' ) ) {
				$cf_weight = 700;
				$cf_name = substr( $name, 0, strrpos( $name, '-' ) );
			} elseif ( false !== strpos( $name, '-800' ) || false !== strpos( $name, '-extrabold' ) || false !== strpos( $name, '-ultrabold' ) ) {
				$cf_weight = 800;
				$cf_name = substr( $name, 0, strrpos( $name, '-' ) );
			} elseif ( false !== strpos( $name, '-900' ) || false !== strpos( $name, '-black' ) ) {
				$cf_weight = 900;
				$cf_name = substr( $name, 0, strrpos( $name, '-' ) );
			} elseif ( false !== strpos( $name, '-600' ) || false !== strpos( $name, '-semibold' ) ) {
				$cf_weight = 600;
				$cf_name = substr( $name, 0, strrpos( $name, '-' ) );
			} elseif ( false !== strpos( $name, '-500' ) || false !== strpos( $name, '-medium' ) ) {
				$cf_weight = 500;
				$cf_name = substr( $name, 0, strrpos( $name, '-' ) );
			} elseif ( false !== strpos( $name, '-400' ) || false !== strpos( $name, '-regular' ) ) {
				$cf_weight = 400;
				$cf_name = substr( $name, 0, strrpos( $name, '-' ) );
			} elseif ( false !== strpos( $name, '-300' ) || false !== strpos( $name, '-light' ) ) {
				$cf_weight = 300;
				$cf_name = substr( $name, 0, strrpos( $name, '-' ) );
			} elseif ( false !== strpos( $name, '-200' ) || false !== strpos( $name, '-extralight' )|| false !== strpos( $name, '-ultralight' )  ) {
				$cf_weight = 200;
				$cf_name = substr( $name, 0, strrpos( $name, '-' ) );
			} elseif ( false !== strpos( $name, '-100' ) || false !== strpos( $name, '-thin' ) ) {
				$cf_weight = 100;
				$cf_name = substr( $name, 0, strrpos( $name, '-' ) );
			}
			if ( false !== strpos( $name, 'italic' ) ) {
				$cf_style = 'italic';
				if ( false !== strpos( $cf_name, '-italic' ) ) {
					$cf_name = substr( $cf_name, 0, strrpos( $cf_name, '-' ) );
				}
			...
			$css .= "font-weight: {$cf_weight};";

			$css .= "font-style: {$cf_style};";
